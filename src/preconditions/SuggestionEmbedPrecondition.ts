...
export class SuggestionEmbedPrecondition extends Precondition {
    ...
    private checkInteraction(interaction: ContextMenuCommandInteraction) {
        ...
        const suggestionStatus =
            SuggestionUtils.parseSuggestionStatusFromColor(embed);
        if (
            this.checkStatusAndCommandMatch(
                suggestionStatus,
                interaction.commandName
            )
        )
            return this.error({
                message: `This suggestion is already ${
                    this.translateSuggestionStatus(suggestionStatus)[1]
                }`,
            });
        if (!interaction.targetMessage.editable)
            return this.error({
                message: `This suggestion cannot be marked as ${
                    this.translateSuggestionStatus(suggestionStatus)[1]
                } by the bot because it doesn't have the appropriate permissions`,
            });
        return this.ok();
    }
    private checkStatusAndCommandMatch(
        status: SuggestionUtils.SuggestionStatus,
        command: string
    ): boolean {
        return this.translateSuggestionStatus(status)[0] == command;
    }
    private translateSuggestionStatus(
        status: SuggestionUtils.SuggestionStatus
    ): [string, string] {
        switch (status) {
