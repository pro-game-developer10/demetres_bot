...
export class TicketChannelPrecondition extends Precondition {
    public override async messageRun(message: Message) {
        return this.checkChannel((message.channel as GuildChannel).name)
            ? this.ok()
            : this.error({
    ...
    public override async chatInputRun(
        ...
        return this.checkChannel((interaction.channel as GuildChannel).name)
            ? this.ok()
            : this.error({
                message: "This is not a ticket channel",
    ...
    public checkChannel(name: string) {
        const ticketChannelNamingPattern =
            /([^a-z0-9])〢([a-z0-9])+-(ticket|donate|job|support)/;
