...
export class TicketChannelPrecondition extends Precondition {
    public override async messageRun(message: Message) {
        return this.checkChannel((message.channel as GuildChannel).name) ? this.ok() : this.error({ message: "This is not a ticket channel", context: { isTicketValidationError: true, message } })
    }
    public override async chatInputRun(interaction: ChatInputCommandInteraction) {
        return this.checkChannel((interaction.channel as GuildChannel).name) ? this.ok() : this.error({ message: "This is not a ticket channel", context: { isTicketValidationError: true, interaction } })
    }
