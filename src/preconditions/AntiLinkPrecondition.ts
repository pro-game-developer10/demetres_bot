...
export class AntiLinkPrecondition extends Precondition {
    public override async messageRun(message: Message) {
        return this.antiLinkPreconditionResults(
            AntiLinkPrecondition.checkMessage(message.content),
            message
        );
    }
    public override async chatInputRun(
        ...
        return this.antiLinkPreconditionResults(
            AntiLinkPrecondition.checkMessage(
    ...
    public static checkMessage(messageContent: string): AntiLinkResults {
        const strippedMessagePieces = messageContent
            .replace("https://", "")
            .replace("http://", "")
            .replace("www.", "")
            .replace("[", " ")
            ...
            .replace(")", " ")
            .split(" ");
        const pieceActions = strippedMessagePieces.map((piece) => {
            if (piece.startsWith("tenor.com")) return AntiLinkResults.PASS;
            if (piece.startsWith("cdn.discordapp.com"))
                return AntiLinkResults.PASS;
            if (piece.startsWith("discord.com/api/v10/webhooks"))
                return AntiLinkResults.PASS;
            if (
            ...
            if (piece.startsWith("discord.com/api/oauth2/authorize/"))
                return AntiLinkResults.TIMEOUT_2H;
            return AntiLinkResults.PASS;
        });
        return pieceActions.reduce((prev, val) =>
            prev > val ? prev : val
        );
    }
    public static antiLinkAction(
        inputResults: AntiLinkResults,
        msg: Message | ChatInputCommandInteraction
    ): () => Promise<MessageAdapter> {
        const errorMsg = `Anti-Link policy violation (Action: ${inputResults})`;
        switch (inputResults) {
        case AntiLinkResults.PASS:
            return async () => {
                return new MessageAdapter(msg);
            };
        case AntiLinkResults.DEFAULT:
            return async () => {
                const adaptee = new MessageAdapter(msg);
                if (
                    (adaptee.subject as ChatInputCommandInteraction)
                        .commandName
                )
                    return adaptee;
                if (adaptee.user.bot) return adaptee;
                if (!adaptee.user.dmChannel)
                    adaptee.user.createDM().then((dm) => {
                        dm.send(errorMsg);
                    });
        ...
        case AntiLinkResults.TIMEOUT_30M:
            return async () => {
                ...
                const member = adaptee.member as GuildMember;
                if (member.moderatable)
                    await member.timeout(Time.Minute * 30, errorMsg);
                await adaptee.deleteReply();
                return adaptee;
            };
        case AntiLinkResults.TIMEOUT_2H:
            return async () => {
                ...
                const member = adaptee.member as GuildMember;
                if (member.moderatable)
                    await member.timeout(Time.Hour * 2, errorMsg);
                await adaptee.deleteReply();
                return adaptee;
            };
    ...
    private antiLinkPreconditionResults(
        inputResults: AntiLinkResults,
        msg: Message | ChatInputCommandInteraction
    ) {
        switch (inputResults) {
        ...
        default:
            ...
            return this.error({
                ...
                context: {
                    action: inputResults,
                    actionExecutable: AntiLinkPrecondition.antiLinkAction(
                        inputResults,
                        msg
                    ),
                },
    ...
    public static memberShallBypass(
        inputResults: AntiLinkResults,
        member?: GuildMember | null
    ) {
        if (!member) return { canBypass: true, result: AntiLinkResults.PASS };
        if (inputResults == AntiLinkResults.BAN)
            return { canBypass: false, result: inputResults };
        const perms = member.permissions;
        if (perms.has(PermissionFlagsBits.BanMembers, true))
            return { canBypass: true, result: AntiLinkResults.PASS };
        return { canBypass: false, result: inputResults };
    }
}
