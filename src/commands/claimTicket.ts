...
export class ClaimTicketCommand extends Command {
    ...
    public async messageRun(message: Message) {
        await (message.channel as GuildChannel).permissionOverwrites.set(TicketPermissionsInfo.ClaimedTicketPermissions(message.author.id,message.guild!))
        return await message.reply(`This ticket is now claimed by <@${message.author.id}>`)
    }
    public async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
        ...
        return await interaction.reply(`This ticket is now claimed by <@${interaction.user.id}>`)
    }
}