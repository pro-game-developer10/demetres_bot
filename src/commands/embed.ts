...
export class EmbedCommand extends Command {
    ...
    public async messageRun(message: Message, args: Args) {
        ...
        const profileArg = await args.pick("enum", EmbedVerificationUtils.getPossibleAuthorProfileValuesAsMapped()).catch(() => "none")
        options.profile = EmbedVerificationUtils.verifyAuthorProfile(profileArg).profile
        const colorArg = await args.pick("enum", { red: "red" }).catch(() => "none")
        options.colorType = EmbedVerificationUtils.verifyColorType(colorArg).profile
        options.description = args.getOption("description")?.replaceAll("\\n","\n").replaceAll("\\_","+()|+").replaceAll("_"," ").replaceAll("+()|+","_") ?? undefined
        options.footer = args.getOption("footer")?.replaceAll("`\\n`","\n").replaceAll("\\_","+()|+").replaceAll("_"," ").replaceAll("+()|+","_") ?? undefined
        options.title = args.getOption("title")?.replaceAll("`\\n`","\n").replaceAll("\\_","+()|+").replaceAll("_"," ").replaceAll("+()|+","_") ?? undefined
